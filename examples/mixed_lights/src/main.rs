...
fn main() {
    ...
    install_common_plugins(
        &mut app,
        "saddle-rendering-2d-lighting — mixed lights",
        "LIGHTING_2D_EXIT_AFTER_SECONDS",
    );
    app.insert_resource(ExampleSceneMode(ExampleMode::MixedLights));
    app.insert_resource(ExampleSceneText {
        title: "2D lighting — mixed lights".into(),
        subtitle: "Point, spot, and texture light authoring in one composition.".into(),
    });
