...
fn main() {
    ...
    install_common_plugins(
        &mut app,
        "saddle-rendering-2d-lighting — occluders",
        "LIGHTING_2D_EXIT_AFTER_SECONDS",
    );
    app.insert_resource(ExampleSceneMode(ExampleMode::Occluders));
    app.insert_resource(ExampleSceneText {
        title: "2D lighting — occluders".into(),
        subtitle: "Rectangle, circle, and polygon occluder authoring with debug outlines.".into(),
    });
